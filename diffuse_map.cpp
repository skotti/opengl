...
const GLchar* cubeFragmentShader = "#version 330 core\n"
	...
	"{\n"
        ...
        "vec3 diffuse = light.diffuse * dott * vec3(texture(material.diffuse, Texcoords));\n"
        "vec3 refdir = reflect(dir, norm);\n"
        "vec3 viewdir = normalize(viewPos - worldVert);\n"
        "float dotspec = pow(max(dot(refdir, viewdir), 0.0f), 32);\n"
        "vec3 specf = light.specular * dotspec * vec3(texture(material.diffuse, Texcoords));\n"
        "color =  vec4(ambient + diffuse + specf, 1.0f); \n"
        "}\n\0";
 ...
 //   glUniform1i(glGetUniformLocation(cubeshaderProgram, "material.diffuse"), 0);
    ...
    while (!glfwWindowShouldClose(window))
    {
      ...
      // Check if any events have been activiated (key pressed, mouse moved etc.) and call corresponding response functions
        ...
        //----------------------------------------------------------------------------------set color
        GLuint lightposLoc = glGetUniformLocation(cubeshaderProgram, "light.position");
        ...
        glUniform3f(lightposLoc, 1.2f, 0.0f, 2.0f);
        ...
        //declare model after appropriate vao is binded
        glm::mat4 cubemodel;
        ...
        glm::mat4 lampmodel;
        lampmodel = glm::translate(lampmodel, glm::vec3(1.2f, 0.0f, 2.0f));
